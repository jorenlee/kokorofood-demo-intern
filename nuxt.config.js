...
export default {
     ...
     sitemap: {
          ...
          routes: async () => {
               const getUrls = async (url, pathPrefix) => {
                    let urls = [];
                    while (url) {
                      const response = await axios.get(url);
                      const data = response.data;
                      urls = urls.concat(data.results.map(item => `${pathPrefix}${item.id}`));
                      url = data.next;
                    }
                    return urls;
                  };
              
                  const productUrls = await getUrls('https://api.kokorofoods.cl/api/main/product/?store=1', '/es/productos/');
                  const blogUrls = await getUrls('https://api.kokorofoods.cl/api/main/blogs/?store=1', '/es/blogs/');
