<template>
    <main role="main">
        ...
        <div class="row featurette">
            ...
            <h2 class="featurette-heading">{{blog.blog_title}}</h2>
...
export default {
  async asyncData({ $axios }) {
    const blogsData = await $axios.$get("/api/main/blogs/");
    return { blogsData };
  },
  auth: "guest",
  data() {
  ...
  async mounted() {
    try {
      ...
      const response = await axios.get(
        `http://api.kokorofoods.cl/api/main/blogs/?id=${this.$route.params.id}`
      );
